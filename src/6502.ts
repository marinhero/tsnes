...
// Flags bits
const CARRY_FLAG = 0; // Carry
const ZERO_FLAG = 1; // Zero
const INTERRUPT_DISABLE_FLAG = 2; // Interrupt Disable
const DECIMAL_MODE_FLAG = 3; // Decimal Mode (Not used in NES)
const BREAK_COMMAND_FLAG = 4; // Break Command
const UNUSED_FLAG = 5; // Unused
const OVERFLOW_FLAG = 6; // Overflow
const NEGATIVE_FLAG = 7; // Negative

function setFlag(cpu: SixFiveZeroTwo, bit: number, value: boolean) {
    if (value) {
        cpu.status |= (1 << bit);
    } else {
        cpu.status &= ~(1 << bit);
...
function isFlagSet(cpu: SixFiveZeroTwo, bit: number): boolean {
    return (cpu.status & (1 << bit)) !== 0;
}
